                ...
                using (var transaction = conn.BeginTransaction())
                {
                    using (SqlCommand cmd = conn.CreateCommand())
                    {
                        ...
                        try
                        {
                            ...
                            string classes_capaCreation = "CREATE TABLE Classes_Capabilities(\r\nClasses_SystemID int,\r\nClassName varchar(50),\r\nCapabilities_SystemID int,\r\nCapabilityName varchar(100),\r\nLevelGained INT,\r\nPRIMARY KEY (Classes_SystemID, ClassName, Capabilities_SystemID, CapabilityName),\r\nCONSTRAINT Classes_Capa_Classes FOREIGN KEY (Classes_SystemID, ClassName) REFERENCES Classes(SystemID, ClassName),\r\nCONSTRAINT Classes_Capa_Capa FOREIGN KEY (Capabilities_SystemID, CapabilityName) REFERENCES Capabilities(SystemID, CapabilityName) ON DELETE CASCADE ON UPDATE CASCADE\r\n)";
                            ...
                            string classesDelTrigger = "CREATE TRIGGER Classes_Delete_Cascade ON Classes\r\nINSTEAD OF DELETE\r\nNOT FOR REPLICATION\r\nAS\r\nBEGIN\r\n\tDELETE FROM Classes_Capabilities FROM Classes_Capabilities JOIN deleted d ON Classes_Capabilities.ClassName=d.ClassName AND Classes_Capabilities.Classes_SystemID=d.SystemID\r\n\tDELETE FROM Classes FROM Classes JOIN deleted d ON Classes.ClassName=d.ClassName AND Classes.SystemID=d.SystemID\r\nEND";
                            ...
                            string classesUpdateTrigger = "CREATE TRIGGER Classes_Update_Cascade ON Classes\r\nINSTEAD OF UPDATE\r\nAS\r\nBEGIN\r\n\tIF(UPDATE(ClassName) AND ((SELECT inserted.ClassName FROM inserted) != (SELECT deleted.ClassName FROM deleted)))\r\n\t\tBEGIN\r\n\t\t\tINSERT INTO Classes(SystemID, ClassName) VALUES((SELECT SystemID FROM inserted),(SELECT ClassName FROM inserted))\r\n\t\t\tUPDATE Classes_Capabilities SET ClassName=(SELECT inserted.ClassName FROM inserted) WHERE Classes_SystemID = (SELECT deleted.SystemID FROM deleted) AND ClassName=(SELECT deleted.ClassName FROM deleted)\r\n\t\t\tUPDATE Characters_Classes SET ClassName=(SELECT inserted.ClassName FROM inserted) WHERE Classes_SystemID = (SELECT deleted.SystemID FROM deleted) AND ClassName=(SELECT deleted.ClassName FROM deleted)\r\n\t\t\tUPDATE Attributes_Classes SET ClassName=(SELECT inserted.ClassName FROM inserted) WHERE Classes_SystemID = (SELECT deleted.SystemID FROM deleted) AND ClassName=(SELECT deleted.ClassName FROM deleted)\r\n\t\t\tDELETE FROM Classes WHERE SystemID=(SELECT deleted.SystemID FROM deleted) AND ClassName=(SELECT deleted.ClassName FROM deleted)\r\n\t\tEND\r\nEND";
                            ...
                            cmd.ExecuteNonQuery();
                            cmd.CommandText = classes_capaCreation;
                            cmd.ExecuteNonQuery();
                            ...
                            cmd.ExecuteNonQuery();
                            cmd.CommandText = classesDelTrigger;
                            cmd.ExecuteNonQuery();
                            ...
                            cmd.ExecuteNonQuery();
                            cmd.CommandText = classesUpdateTrigger;
                            cmd.ExecuteNonQuery();
                            transaction.Commit();
                            #endregion
                        }
                ...
                using (var transaction = conn.BeginTransaction())
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        try
                        {
                            ...
                            string classesDelTrigger = "CREATE TRIGGER Classes_Delete_Cascade BEFORE DELETE ON Classes\r\nBEGIN\r\n\tDELETE FROM Classes_Capabilities WHERE Classes_Capabilities.ClassName=OLD.ClassName AND Classes_Capabilities.Classes_SystemID=OLD.SystemID;\r\nEND;";
                            ...
                            string classesUpdateTrigger = "CREATE TRIGGER Classes_Update_Cascade BEFORE UPDATE OF ClassName ON Classes\r\nWHEN NEW.ClassName!=OLD.ClassName\r\nBEGIN\r\n\tINSERT INTO Classes(SystemID, ClassName) VALUES(NEW.SystemID,NEW.ClassName);\r\n\tUPDATE Classes_Capabilities SET ClassName=NEW.ClassName WHERE Classes_SystemID = OLD.SystemID AND ClassName=OLD.ClassName;\r\n\tUPDATE Characters_Classes SET ClassName=NEW.ClassName WHERE Classes_SystemID = OLD.SystemID AND ClassName=ClassName=OLD.ClassName;\r\n\tUPDATE Attributes_Classes SET ClassName=NEW.ClassName WHERE Classes_SystemID = OLD.SystemID AND ClassName=ClassName=OLD.ClassName;\r\n\tDELETE FROM Classes WHERE SystemID=OLD.SystemID AND ClassName=OLD.ClassName;\r\nEND";
